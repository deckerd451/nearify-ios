...
/// Privacy: limits identity exposure unless connected.
struct EncounterCardView: View {
    ...
    var body: some View {
        FeedCardContainer {
            ...
            HStack(spacing: 12) {
                feedActionButton("Connect", icon: "person.badge.plus", color: .green) {
                    onConnect?()
                }
                feedActionButton("Dismiss", icon: "xmark", color: .gray) {
                    onDismiss?()
                }
            }
            .padding(.top, 12)

...
struct NetworkView: View {
    ...
    private var reconnectBanner: some View {
        if let ctx = eventJoin.reconnectContext {
            VStack(spacing: 12) {
                ...
                HStack(spacing: 12) {
                    Button {
                        Task { await eventJoin.joinEvent(eventID: ctx.eventId) }
                    } label: {
                        Text("Rejoin").font(.subheadline).fontWeight(.semibold)
                            .foregroundColor(.white).frame(maxWidth: .infinity)
                            .padding(.vertical, 10).background(Color.blue).cornerRadius(10)
                    }
                    Button {
    ...
    private var exitedState: some View {
        ScrollView {
            VStack(spacing: 24) {
                ...
                VStack(spacing: 16) {
                    stateBanner
                    if let n = modeState.membership.eventName {
                        Text(n).font(.headline).foregroundColor(.white.opacity(0.6))
                    }
                    exitExplanationText
        ...
        ScrollView {
            VStack(spacing: 0) {
                ...
                if displayAttendees.isEmpty { emptyState }
                    ...
                    else { attendeeListView.padding(.horizontal) }
                }
            }
        }
    }
        ...
        VStack(spacing: 8) {
            if let en = eventJoin.currentEventName ?? presence.currentEvent {
                ...
                HStack(spacing: 4) {
                    ...
                    Text("Live at this event").font(.caption2).foregroundColor(.green.opacity(0.8))
                }
            }
            stateBanner
            HStack(spacing: 16) {
            ...
            if case .inEvent = modeState.membership {
                Button { showLeaveConfirmation = true } label: {
                    ...
                    .foregroundColor(.red.opacity(0.8)).padding(.horizontal, 12).padding(.vertical, 6)
    ...
    private var topPeopleSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Text("Top People Right Now").font(.headline).foregroundColor(.white)
                Spacer()
                if intelligence.isLoading { ProgressView().scaleEffect(0.6).tint(.gray) }
            }
            if intelligence.topPeople.isEmpty {
                VStack(spacing: 6) {
                    HStack(spacing: 8) {
                        Image(systemName: "figure.walk").foregroundColor(.gray)
                        Text("No strong interactions right now").font(.caption).foregroundColor(.gray)
                    }
                    Text("Move around to discover people nearby").font(.caption2).foregroundColor(.gray.opacity(0.6))
                }.padding(.vertical, 4)
            } else {
                VStack(spacing: 8) { ForEach(intelligence.topPeople) { p in topPersonRow(p) } }
            }
    ...
    private func topPersonRow(_ person: RankedProfile) -> some View {
        ...
        let ac: Color = {
            if let t = d?.tier {
                switch t {
                case .activeConversation: return .blue; case .strongInteraction: return .green
                case .breakthroughPotential: return .cyan; case .repeatedNearMiss: return .orange
                case .followUpGap: return .purple; case .fallback: return .gray
                }
            }
            switch ins?.needState {
            case .belonging: return .orange; case .esteem: return .purple
            case .selfActualization: return .cyan; case .none: return .blue
            }
        }()
        return HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 4) {
                    Text(person.name).font(.subheadline).fontWeight(.medium).foregroundColor(.white)
                    if person.isConnected { Image(systemName: "link").font(.system(size: 9)).foregroundColor(.green) }
                    if person.hasMessaged { Image(systemName: "bubble.left.fill").font(.system(size: 9)).foregroundColor(.blue) }
                }
                if let r = d?.reason { Text(r).font(.caption2).foregroundColor(ac.opacity(0.9)).lineLimit(2) }
                else if let it = ins?.insightText { Text(it).font(.caption2).foregroundColor(ac.opacity(0.9)).lineLimit(2) }
                else if person.encounterStrength > 0 {
                    let m = person.encounterStrength / 60
                    Text(m > 0 ? "\(m)m nearby" : "\(person.encounterStrength)s nearby").font(.caption2).foregroundColor(.orange)
                }
    ...
    private func nearbyDeviceRow(_ device: DiscoveredBLEDevice) -> some View {
        ...
        return HStack(spacing: 10) {
            ...
            if device.name.hasPrefix("BCN-") { deviceTag("ATTENDEE", color: .cyan) }
            else if device.name.contains("MOONSIDE") || device.isKnownBeacon { deviceTag("ANCHOR", color: .blue) }
    ...
    private func deviceTag(_ t: String, color: Color) -> some View {
        Text(t).font(.system(size: 9, weight: .bold)).foregroundColor(.black).padding(.horizontal, 5).padding(.vertical, 2).background(color).cornerRadius(4).fixedSize()
    }

    private var emptyState: some View {
        VStack(spacing: 20) {
            Spacer(minLength: 32)
            Image(systemName: "person.crop.circle.badge.questionmark").font(.system(size: 60)).foregroundColor(.gray)
            Text("You're the first one here").font(.title3).fontWeight(.semibold).foregroundColor(.white)
        ...
        GeometryReader { geo in
            ...
            ZStack {
                ForEach(Array(ps.enumerated()), id: \.element.attendee.id) { i, p in
                    let r = radiusForAttendee(p.attendee, in: geo.size)
                    let pos = radialPosition(index: i, total: ps.count, center: c, radius: r)
                ...
                Circle().fill(Color.blue).frame(width: 50, height: 50)
                    .overlay(Text("You").font(.caption).fontWeight(.bold).foregroundColor(.white)).position(c)
                ForEach(Array(ps.enumerated()), id: \.element.attendee.id) { i, p in
                    let r = radiusForAttendee(p.attendee, in: geo.size)
                    let pos = radialPosition(index: i, total: ps.count, center: c, radius: r)
                    let sz = nodeSize(for: p.attendee)
        ...
        VStack(spacing: 3) {
            ZStack {
                ...
                Circle().fill(pres.nodeColor).frame(width: size, height: size)
                    .overlay(Text(pres.attendee.initials).font(.caption).fontWeight(.bold).foregroundColor(.white)).opacity(pres.nodeOpacity)
                if pres.relationship == .connected {
                    Image(systemName: "link").font(.system(size: 8, weight: .bold)).foregroundColor(.black)
                        .padding(3).background(Circle().fill(pres.ringColor)).offset(x: size / 2 - 2, y: -(size / 2 - 2))
                }
            }
    ...
    private var attendeeListView: some View {
        LazyVStack(spacing: 12) {
            ForEach(displayAttendees) { a in
                Button(action: { selectedAttendee = a }) { AttendeeCardView(attendee: a) }.buttonStyle(.plain)
            }
        }.padding(.vertical, 16)
    }
        ...
        NavigationView {
            Form {
                ...
                Section(header: Text("Diagnostics")) {
                    ...
                    Button(action: { Task { await EventPresenceService.shared.debugWritePresenceNow(); await MainActor.run { showPresenceTestResult = true } } }) {
    ...
    private func proximityScore(for a: EventAttendee) -> Double {
        guard let d = stateResolver.peerDevice(for: a) else { return 0.5 }
        let r = scanner.smoothedRSSI(for: d.id) ?? d.rssi
        switch r { case -45...0: return 1.0; case -55..<(-45): return 0.8; case -65..<(-55): return 0.6; case -75..<(-65): return 0.4; default: return 0.25 }
    }
    private func radiusForAttendee(_ a: EventAttendee, in s: CGSize) -> CGFloat {
        let mn = min(s.width, s.height) * 0.18; let mx = min(s.width, s.height) * 0.36
        return mx - (mx - mn) * CGFloat(proximityScore(for: a))
    }
    ...
    private func radialPosition(index: Int, total: Int, center: CGPoint, radius: CGFloat) -> CGPoint {
        ...
        let a = (Double(index) / Double(total)) * 2.0 * .pi
    ...
    private var nearbyDevices: [DiscoveredBLEDevice] {
        scanner.getFilteredDevices().filter { $0.isKnownBeacon || $0.name.hasPrefix("BEACON-") || $0.name.hasPrefix("BCN-") }.sorted { $0.rssi > $1.rssi }
    }
    private func deviceColor(for d: DiscoveredBLEDevice) -> Color {
        if d.name.hasPrefix("BCN-") { return .cyan }; if d.name.hasPrefix("BEACON-") { return .green }; if d.isKnownBeacon { return .orange }; return .gray
    }
    private func resolvedAttendeeName(for d: DiscoveredBLEDevice) -> String? {
        guard let p = BLEAdvertiserService.parseCommunityPrefix(from: d.name) else { return nil }
        return displayAttendees.first { String($0.id.uuidString.prefix(8)).lowercased() == p }?.name
    }
...
private extension View {
    func flexibleFrame(minWidth: CGFloat, maxWidth: CGFloat) -> some View { self.frame(minWidth: minWidth, maxWidth: maxWidth) }
}
